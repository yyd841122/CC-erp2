...
### 44. 商品分类后端API与Excel导入修复
**请求**: 修复商品分类Excel导入和前端显示问题
**状态**: ✅ 已完成 (v1.3.1)
**详情**:
- 修复 `CategoryDialog.vue` Excel导入列映射问题
- 修复 `ProductList.vue` 分类列表加载问题
- 修复商品导入时分类名称未保存问题
- 优化 Excel 模板生成格式
- 添加列标题和说明工作表

**修复文件**:
- `cc-erp-frontend/src/views/product/CategoryDialog.vue`
- `cc-erp-frontend/src/views/product/ProductList.vue`
- `cc-erp-frontend/src/utils/excel.js`

---

### 45. 商品管理批量操作功能
**请求**: 商品管理页面添加批量删除和批量更改分类功能
**状态**: ✅ 已完成 (v1.4.2)
**详情**:
- 添加批量操作工具栏（勾选时显示）
- 批量删除功能（可删除多个选中商品）
- 批量更改分类功能（统一修改选中商品的分类）
- 添加批量操作对话框
...
### 47. 商品管理Excel导入去重优化
**请求**: 导入Excel时检查重复数据，只导入新数据
**状态**: ✅ 已完成 (v1.4.2)
**详情**:
- 导入前获取全部商品数据
- 去重逻辑：**商品名称 + 分类 + 规格** 三者相同视为重复
- 跳过重复数据，只导入新数据
- 显示跳过数量统计
...
**状态**: ✅ 已完成 (v1.4.2)
**详情**:
- 商品名称列从固定宽度改为 `min-width="150"` 自适应
- 添加 `show-overflow-tooltip` 属性显示完整内容悬浮提示
- 所有列使用 `min-width` 替代固定 `width`，支持比例自适应
...
- `src/views/product/ProductList.vue` - Excel导入导出、批量操作、表头排序、列宽自适应
- `src/views/product/ProductDialog.vue` - 自动生成编码、单位默认值
- `src/views/product/CategoryDialog.vue` - 分类管理、Excel导入优化
