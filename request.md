...
### 57. 修复管理员权限加载和登录问题
**请求**: 创建管理员后登录，提示"您没有权限执行此操作"
**状态**: ✅ 已完成
**详情**:

**问题原因**:
1. `CustomUserDetailsService` 没有从数据库加载用户权限，使用空的权限列表
2. `SecurityConfig` 中的公开接口路径配置错误（`/auth/login` 应为 `/v1/auth/login`）
3. `SysUserMapper.xml` 中的权限查询引用了不存在的 `is_deleted` 字段
4. `init_user.sql` 中的密码哈希不正确
5. 登录页面存在模拟模式复选框，需要移除

**修复内容**:

**cc-erp-backend/src/main/java/com/cc/erp/mapper/SysUserMapper.java**:
- 添加 `findUserPermissions(Long userId)` 方法 - 查询用户权限代码列表

**cc-erp-backend/src/main/resources/mapper/SysUserMapper.xml**:
- 添加查询用户权限的 SQL（从 sys_user_role → sys_role_permission → sys_permission 关联查询）
- 移除不存在的 `sp.is_deleted` 条件

**cc-erp-backend/src/main/java/com/cc/erp/security/CustomUserDetailsService.java**:
- `loadUserByUsername()` 现在从数据库加载用户权限
- 权限为空时授予默认的 ROLE_ADMIN 角色

**cc-erp-backend/src/main/java/com/cc/erp/config/SecurityConfig.java**:
- 修正公开接口路径：`/auth/login` → `/v1/auth/login`
- 添加端口 5176 到 CORS 允许列表

**cc-erp-frontend/src/views/Login.vue**:
- 移除"使用模拟数据模式"复选框及相关代码
- 移除模拟模式标签显示
- 移除 `isMockMode` computed 属性
- 移除 `handleMockModeChange` 方法

**init_user.sql**:
- 更新为正确的 BCrypt 密码哈希（`admin123` → `$2a$10$EnwIb5wRdnfJVoCV2FNP..ftPgmKXm1C4BYS.0gSgnIgKmDq6453S`）
- 更新角色代码为 `ROLE_ADMIN`
- 更新用户真实姓名为"系统管理员"

**cc-erp-backend/src/main/java/com/cc/erp/util/PasswordHashGenerator.java**:
- 创建密码哈希生成工具，用于生成正确的 BCrypt 哈希

**效果**:
- 管理员登录成功，返回 token 和用户信息
- 权限从数据库正确加载
- 登录页面界面更简洁，无模拟模式选项
...
### 58. 商品分类管理后端API实现
**请求**: 分类管理导入数据，提示成功，但没有正常显示，检查下原因，并且修复，查下数据库中有没有存入数据
**状态**: ✅ 已完成
**详情**:

**问题原因**:
1. 商品分类没有后端 API，前端使用 localStorage 存储
2. 导入数据只保存到 localStorage，没有存入数据库
3. ProductController 缺少创建/更新/删除方法
4. 缺少客户/供应商的 CRUD 权限（base:customer:create/update/delete, base:supplier:create/update/delete）

**修复内容**:

**后端新增文件**:
- `cc-erp-backend/src/main/java/com/cc/erp/entity/BizProductCategory.java` - 商品分类实体
- `cc-erp-backend/src/main/java/com/cc/erp/mapper/BizProductCategoryMapper.java` - 分类Mapper接口
- `cc-erp-backend/src/main/resources/mapper/BizProductCategoryMapper.xml` - 分类SQL映射
- `cc-erp-backend/src/main/java/com/cc/erp/controller/ProductCategoryController.java` - 分类控制器

**数据库权限更新**:
- 添加 6 个新权限到 `sys_permission` 表：
  - `base:customer:create/update/delete`
  - `base:supplier:create/update/delete`
- 分配给管理员角色 (role_id = 1)
...
**前端 API 更新** ([src/api/product.js](e:\CC_ERP\cc-erp-frontend\src\api\product.js)):
- 添加商品分类 API：`getCategoryList()`, `createCategory()`, `updateCategory()`, `deleteCategory()`
- 自动识别模拟模式，优先使用后端 API

**前端组件更新** ([src/views/product/CategoryDialog.vue](e:\CC_ERP\cc-erp-frontend\src\views\product\CategoryDialog.vue)):
- 从 localStorage 改用后端 API
- 导入功能调用 `createCategory()` API

**效果**:
- 分类数据保存到 PostgreSQL 数据库
- 支持 Excel 导入分类（调用后端 API）
- 所有模块的 Excel 导入功能正常工作
...
- `cc-erp-backend/src/main/java/com/cc/erp/util/PasswordHashGenerator.java` - **密码哈希生成工具**
- `cc-erp-backend/src/main/java/com/cc/erp/config/SecurityConfig.java` - **修正路由配置和CORS设置**
- `cc-erp-backend/src/main/java/com/cc/erp/security/CustomUserDetailsService.java` - **从数据库加载用户权限**
- `cc-erp-backend/src/main/java/com/cc/erp/mapper/SysUserMapper.java` - **添加权限查询方法**
- `cc-erp-backend/src/main/resources/mapper/SysUserMapper.xml` - **添加权限查询SQL**
- `cc-erp-erp-frontend/src/views/Login.vue` - **移除模拟模式复选框**
- `init_user.sql` - **使用正确的BCrypt密码哈希**
- `cc-erp-backend/src/main/java/com/cc/erp/entity/BizProductCategory.java` - **商品分类实体类**
- `cc-erp-backend/src/main/java/com/cc/erp/mapper/BizProductCategoryMapper.java` - **商品分类Mapper**
- `cc-erp-backend/src/main/resources/mapper/BizProductCategoryMapper.xml` - **商品分类SQL映射**
- `cc-erp-backend/src/main/java/com/cc/erp/controller/ProductCategoryController.java` - **商品分类控制器**
- `cc-erp-backend/src/main/java/com/cc/erp/controller/ProductController.java` - **添加商品创建/更新/删除方法**
- `cc-erp-frontend/src/api/product.js` - **添加商品分类API**
- `cc-erp-frontend/src/views/product/CategoryDialog.vue` - **使用后端API**
...
- ✅ 修复管理员密码哈希问题
- ✅ 移除登录页面模拟模式复选框
- ✅ 更新 init_user.sql 使用正确的 BCrypt 密码哈希
